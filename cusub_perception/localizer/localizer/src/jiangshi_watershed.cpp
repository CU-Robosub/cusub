...
namespace pose_generator
{
    JiangshiWatershed::JiangshiWatershed()
    {
        ...
        if (!nh.getParam("localizer/jiangshi_watershed/use_aspect_ratio", useAspectRatio))
        {
            ROS_ERROR("Jiangshi couldn't locate params");
            abort();
        }
        if (!useAspectRatio)
        {
            ROS_WARN("Jiangshi Watershed not using bb aspect ratio");
        }
        else
        {
            ROS_INFO("Jiangshi Watershed using bb aspect ratio");
        }
    ...
    bool JiangshiWatershed::getPoints(Mat &img, int border_size, vector<Point2f> &points)
    {
        ...
        if ((img.cols - 2 * border_size < 4) || (img.rows - 2 * border_size < 4))
        {
            return false;
        } // min rectangle width 4 pixels
    ...
    void JiangshiWatershed::getOrientationFromAspectRatio(darknet_ros_msgs::BoundingBox &bb, float horizontalDist, geometry_msgs::Quaternion &quat)
    {
        ...
        { // on the left of us, all angles should be positive
            m = 90 / (aspectRatio90Deg - 0.5);
            b = 90 - m * aspectRatio90Deg;
        }
        ...
        vector<localizer_msgs::Detection> &detections)
    {
        ...
        if (!getPoints(img, border_size, img_points))
        {
            return false;
        }
        ...
        if (useAspectRatio)
        {
            getOrientationFromAspectRatio(bbs[0], pose.position.x, pose.orientation);
        }
        return true;
    }
}