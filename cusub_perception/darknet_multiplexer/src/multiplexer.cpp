...
namespace darknet_multiplexer_ns
{
    ...
    void Multiplexer::init(ros::NodeHandle& nh)
    {
        ...
        {
            cuprint_warn("failed to locate params.");
            abort();
        }
...
// Individual callbacks for speed, no logic about where to store images
    ...
    void Multiplexer::cuprint(std::string str)
    {
        std::string print_str = std::string("[\033[92mMultiplexer\033[0m] ");
        ROS_INFO( (print_str + str).c_str());
    }

    void Multiplexer::cuprint_warn(std::string str)
    {
        std::string print_str = std::string("[\033[92mMultiplexer\033[0m] ");
