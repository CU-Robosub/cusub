...
bool YoloObjectDetector::publishOriginalImage(const cv::Mat& originalImage)
{
  ...
  originalImagePublisher_.publish(*cvImage.toImageMsg());
  ROS_DEBUG("Original image has been published.");
  return true;
}
...
void YoloObjectDetector::yolo()
{
  ...
  ipl2_ = cvCreateImage(cvSize(buff_[0].w, buff_[0].h), IPL_DEPTH_8U, buff_[0].c);
...
void *YoloObjectDetector::publishInThread()
{
  ...
  cv::Mat cvImage2 = cv::cvarrToMat(ipl2_);
  if (!publishOriginalImage(cv::Mat(cvImage2))) {
    ROS_DEBUG("Original image has not been broadcasted.");
  }

  // This was used to sync bounding boxes, but we shoved the image in there
  // so its not needed
  //std::this_thread::sleep_for(std::chrono::milliseconds((int)(250.0)));
