...
static int windows_handle_events(struct libusb_context *ctx, struct pollfd *fds, POLL_NFDS_TYPE nfds, int num_ready)
{
	...
	for (i = 0; i < nfds && num_ready > 0; i++) {
		...
		if (found) {
		  ...
		  // * BUG: libusb windows backend leaks fds on disconnect
		  usbi_err(ctx, "could not find a matching transfer for fd %d; freeing it", fds[i].fd);
		  int fd = fds[i].fd;
		  usbi_remove_pollfd(ctx, fd);
		  usbi_free_fd(fd);
...
static int winusb_clear_halt(struct libusb_device_handle *dev_handle, unsigned char endpoint)
{
	...
	WinUsb_AbortPipe(winusb_handle, endpoint);
	WinUsb_FlushPipe(winusb_handle, endpoint);
