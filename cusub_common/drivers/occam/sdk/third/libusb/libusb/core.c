...
libusb_device * LIBUSB_CALL libusb_ref_device(libusb_device *dev)
{
	...
	dev->refcnt++;
	usbi_dbg("ref device %d.%d %d", dev->bus_number, dev->device_address, dev->refcnt);
...
void API_EXPORTED libusb_unref_device(libusb_device *dev)
{
	...
	usbi_mutex_lock(&dev->lock);
	refcnt = --dev->refcnt;
	usbi_dbg("unref device %d.%d %d", dev->bus_number, dev->device_address, dev->refcnt);
	usbi_mutex_unlock(&dev->lock);
...
#if 0
	static FILE *file_stream = 0;
	if (!file_stream)
	  file_stream = fopen("c:\\camerahost\\libusb.log","w");
	stream = file_stream;
	if (!stream)
	  return;
#endif
